    ...
    if(sessDonor){
        ...
        try{
           ...
           const update = await Donor.findById(req.session.donor.id).update({
            ...
            const {pathName} = await generatePdf(name, amount, initiativeName.title, groupName.name)
            console.log(pathName)
            sendGiftEmail(email, pathName, initiativeName.title, userName.name, goalAmount.goalAmount, amount, groupName.name)
...
router.get("/list-status/:groupId", async(req, res)=>{
    ...
    const sessDonor = req.session.donor;
    if (sessDonor){
        const memberQuery = await Donor.findOne({'groupsNotifiedBy._id': groupId})
