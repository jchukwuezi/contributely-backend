...
router.get("/:groupCode/initiatives", async(req, res)=>{
...
router.get("/:groupCode/initiatives/:initiativeId", async (req, res)=>{
...
router.post("/:groupCode/:initiativeId/donate", async(req, res)=>{
    const groupCode = req.params.groupCode
    const initiativeId = req.params.initiativeId
    //finding the group and initiative name for metadata section
    const orgStripeId = await Organisation.findById(groupCode).select({_id:0, stripeAccountId:1})
    const groupName = await Organisation.findById(groupCode).select({_id:0, name:1})
    const initiativeName = await Initiative.findById(initiativeId).select({_id:0, title:1})
