...
const {sendStartEmail} = require('../../services/email')
    ...
    if(sessOrg){
        ...
        Organisation.findOne({_id:req.session.org.id}).then(async (org) => {
            if(!org){
                ...
                const mailingList = await Organsation.findById(req.session.org.id).populate({
                    path: 'notificationList',
                    select: 'email -_id'
                })
                console.log(mailingList)
                console.log(mailingList.notifcationList)
                sendStartEmail(org.name, title, goalAmount, mailingList.notifcationList)
